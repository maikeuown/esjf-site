...
const avisosData = [
  ...
  { id: 2, title: 'Alteração de Horário - Secretaria', excerpt: 'Novo horário a partir de 15 de abril.', priority: 'normal', date: '2025-04-05', slug: 'alteracao-horario-secretaria' },
  { id: 3, title: 'Encerramento da Biblioteca', excerpt: 'Biblioteca encerrada 10 e 11 de abril.', priority: 'low', date: '2025-04-03', slug: 'encerramento-biblioteca' },
];

const newsData = [
  { id: 1, title: 'Olimpíadas da Matemática', excerpt: '120+ alunos na fase regional.', category: 'Académico', color: '#1e40af', date: '2025-03-28', slug: 'olimpiadas-matematica' },
  { id: 2, title: 'Novo Laboratório de Ciências', excerpt: 'Investimento de 150.000€.', category: 'Geral', color: '#059669', date: '2025-04-01', slug: 'novo-laboratorio-ciencias' },
  { id: 3, title: 'Campeões de Futsal', excerpt: 'Vitória no torneio regional.', category: 'Desporto', color: '#d97706', date: '2025-04-03', slug: 'campeoes-futsal' },
];

const eventsData = [
  { id: 1, title: 'Dia Aberto', date: '2025-04-20', location: 'ESJF', slug: 'dia-aberto' },
                  ...
                  <Link key={aviso.id} href={`/avisos/${aviso.slug}`}>
                    <Card className="card-glow-lift-pulse border-border/50 cursor-pointer overflow-hidden">
                      <CardContent className="p-5">
                        ...
                        <h3 className="font-semibold mb-2 line-clamp-2 text-glow hover:text-brand-600 transition-colors">{aviso.title}</h3>
                        {aviso.excerpt && <p className="text-xs text-muted-foreground mb-2 line-clamp-1 text-glow">{aviso.excerpt}</p>}
                ...
                <Link key={item.id} href={`/noticias/${item.slug}`}>
                  <Card className="card-glow-lift-pulse border-border/50 cursor-pointer overflow-hidden">
                    <CardContent className="p-5">
                      ...
                      <h3 className="font-semibold mb-2 line-clamp-2 text-glow hover:text-brand-600 transition-colors">{item.title}</h3>
                      {item.excerpt && <p className="text-xs text-muted-foreground mb-2 line-clamp-1 text-glow">{item.excerpt}</p>}
